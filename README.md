...
It installs, runs `nuxt generate`, and prints `.output/public/index.html`. That
file is a 95 byte meta refresh stub instead of the page. You can also open it in
the file tree.
...
- The database driver is irrelevant to the bug. This repo sets
  `aiReady.database.type` to `d1` so it runs in StackBlitz WebContainer, which
  cannot load the native `better-sqlite3` that the default `sqlite` driver
  requires. The default driver reproduces the exact same stub.
